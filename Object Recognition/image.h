...
class Image {
 public:
  ...
  void ConvertBinary(const int threshold) {
    for(int i = 0; i < num_rows_; i++) {
      for(int k = 0; k < num_columns_; k++) {
        if(pixels_[i][k] < threshold) pixels_[i][k] = 0;
        else pixels_[i][k] = 255;
      }
    }
  }
